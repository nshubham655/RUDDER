...
class CENet(BaseModel):
    ...
    def forward(self, experts, ind, text=None, raw_captions=None, text_token_mask=None, text_labels=None):
...
def sharded_cross_view_inner_product(vid_embds, text_embds, text_weights,
    ...
    if num_caps > 1:
        ...
        if merge_caption_similiarities == "avg":
            ...
            #     zs_dispFig()
            #sims = sims.repeat(num_caps,1,1)
            #sims = sims.view(B*num_caps,-1)
            #sims = sims[:B,:]
            newsims = th.zeros(B,num_caps+B).cuda()
            for i in range(num_caps):
                newsims[:,i] = sims[:,i,:].diag()
            newsims[:,num_caps:] = sims[:,0,:]
