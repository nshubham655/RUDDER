...
class ActivityNet(BaseDataset):
    ...
    def dataset_paths() -> Dict[str, Union[str, List[str], Path, Dict]]:
        ...
        for text_feat in ("openai", ):
            ...
            text_feat_paths[text_feat] = {key: f"aggregated_text_feats/{val}.pickle"
                                          for key, val in text_feat_names.items()}
            challenge_text_feat_paths[text_feat] = \
                f"aggregated_text_feats/{text_feat}.pickle"
        feature_info = {
            ...
            "raw_captions_path": "raw-captions-train-val_1.pickle",
        }
        return feature_info
