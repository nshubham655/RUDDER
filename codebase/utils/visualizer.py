...
class Visualizer:
    ...
    def __init__(self, exp_name, web_dir, src_video_dir, vis_vid_freq, num_samples=50):
        ...
        if sym_dir.is_symlink():
            os.remove(sym_dir)
        print(f"Symlinking {sym_dir} to {src_dir}")
    ...
    def visualize_ranking(self, sims, epoch, meta, nested_metrics):
        ...
        sample = np.random.choice(np.arange(dists.shape[0]), size=100,
                                  replace=False)

        for ii in sample:
            ranked_idx = sorted_ranks[ii][:vis_top_k]
            gt_captions = meta["raw_captions"][ii][:1]

            
            best_captions = []
            for r in ranked_idx:
                best_captions.append(' '.join(meta["raw_captions"][r][0]))
            datum = {
                "gt-sim": -gt_dists[ii],
                ...
                "top-k-captions": np.array(best_captions),
                "hide-gt": hide_gt,
            }
    ...
    def display_current_results(self, rankings, epoch, metrics):
        ...
        for ranking in rankings:
            ...
            for idx, (vid_path, caption, sim) in enumerate(zip(ranking["top-k-paths"], ranking['top-k-captions'],
                                                  ranking["top-k-sims"])):
                vid_path = Path(vid_path)
                if ranking["hide-gt"]:
                    txt = f"choice: {idx}"
                else:
                    txt = f"<b>Rank: {idx}, Sim: {sim:.3f}, [{Path(vid_path).stem}] {caption}"
                txts.append(txt)
